...
<% Title = Session.Contents("strUserName") & "的日记本—编辑" %>
<%
...
If Session.Contents("PN_MUDiaryMaster_" & Session.Contents("strUserName")) And Request.Form("id") <> "" And dtmDateTime <> "" And strWeather <> "" And strTitle <> "" And strContent <> "" Then
	ID = Trim(Request.Form("id"))
	ConnectionDatabase
	strSQL = "Update [Diary] Set dtmDateTime = '"& dtmDateTime &"', strWeather = '"& strWeather &"', strTitle = '"& strTitle &"', strContent = '"& strContent &"', blnSecrecy = '"& blnSecrecy &"', strIP = '"& strIP &"' Where strUserName = '" & Session.Contents("strUserName") & "' And ID=" & Request.Form("id")
	Conn.Execute(strSQL)
	Set strSQL = Nothing
	CloseDatabase
...
If Not(Rs.EOF) Then
%><form name="write" method="post" action="edit.asp">
              日期： 
